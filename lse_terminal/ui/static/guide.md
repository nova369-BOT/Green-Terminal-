...
| RESEARCH | New quant papers with an in app reader, and an analysis desk that counts readings from loaded bars | No |
| MY DATA | Import files, download provider history, manage the library, the ten samples | Only for LSE Data |
...
4. **Open BACKTEST.** Seven starter strategies are seeded into your workspace on first open. Pick one, click a dataset in the library on the left, press RUN, and the report prints in the terminal dock underneath. Use **Save backtest** in a completed report to keep an exact snapshot; saved runs appear under **PAST BACKTESTS** in the library after a restart and can be reopened or deleted there.
...
- **Drawings.** The rail on the left of the chart is six icons, no labels. **Line** is two clicks. **Swing** snaps to the nearest swing high or low and marks that price. **Span** is two clicks and prints the bar count, the point change, and how many typical ranges that move is. **Band** draws one typical bar of range around the price you click. **Wound** marks a close in the outer quarter of its bar and how many bars until price trades back through that bar's open. **Streak** boxes the run of up or down closes the bar belongs to. Every number is counted from the bars loaded on the chart. Drawings are stored per instrument. Escape cancels a drawing in progress. Escape or Backspace deletes the selected one. Lock and hide sit at the bottom of the rail, then remove selected, then remove all when more than one is on the chart.
...
BACKTEST is where strategies are written and tested on your own data. Four modes in the secondary bar: ALGO DEVELOPMENT (a code editor with one click backtests), PORTFOLIO BACKTESTING (combine strategies and instruments with fixed capital allocations), MACHINE LEARNING (train models on your imports, on this machine), and MANUAL (bar replay you trade by hand). Entering the tab switches the data source to your library; nothing here touches live data.

**Portfolio backtesting.** Choose a saved Python strategy and an imported OHLCV dataset for each component. Enter the starting portfolio capital, allocation percentages, strategy parameters and commissions. Allocations may total less than 100%; the remainder stays as cash. Leaving dates blank uses all available history at each dataset's native timeframe. A date-only end includes that full UTC day. The run uses saved strategy source, so save editor edits first.

**Commission.** Choose a cash amount per contract/unit per side for futures or per-share fees, or a percentage of traded notional per side. Enter your broker's total charge, including exchange and clearing fees if applicable. A cash fee of 2.50 means 5.00 for one contract opened and closed; it is not multiplied by the contract's point value. NQ and MNQ can have different fee amounts. Percentage commission does use price × quantity × point value. Zero means no commission is modeled; the app does not guess a broker rate. Individual strategy runs have the same controls; each portfolio component has its own setting.

The report shows gross P&L, total commission and net P&L, including a component breakdown for portfolios. Equity and all performance statistics include commission once, at entry and exit. The trade ledger and CSV include the fees; saved reports preserve both the result and the fee assumptions. Older reports without recorded fee fields show an unavailable breakdown until rerun. Gross P&L uses the strategy's fill prices, so any slippage already modeled in those fills remains included.

Each component runs independently on its own prices with its allocated starting capital. Explicit quantities and contract multipliers remain controlled by the strategy; allocations do not enforce futures margin or leverage limits. The combined report sums equity on a common timeline, includes cash before a component starts, carries its last known mark through gaps, and holds its final equity after its data ends. It recalculates portfolio statistics, shows component contributions and coverage, and tags trades by strategy and instrument. The buy-and-hold comparison uses the same allocated starting cash in each component's normalized price series plus unused cash. Use matching P&L currencies; no FX conversion, shared margin, cross-strategy netting, transfers or rebalancing is simulated.

Use **Save backtest** to preserve the complete portfolio result and submitted strategy sources. Reopen it under **PAST BACKTESTS** after restarting. **Portfolio setup** returns to your current configuration while keeping completed reports in separate tabs. Portfolio robustness checks use combined daily outcomes; run walk-forward and parameter sensitivity separately on each component from its individual strategy report.
...
**Robustness.** A completed strategy report includes a **Robustness** tab, also available when reopening a saved backtest. It shows an early 70% versus latest 30% calendar comparison, yearly returns and daily drawdowns, profit remaining after removing the best 1%, 5% and 10% of closed trades, and an additional trading-cost scenario. Extra cost is entered in account currency per unit per side, so total extra cost is twice the absolute recorded quantity times that amount; existing fees are already included in trade P&L.

The tab also runs 1,000 repeatable **daily block bootstrap** paths from the report's observed daily cash P&L. Choose 1, 5 or 20 observed days per block, a seed, and a drawdown threshold, then click **Apply settings**. The report shows ending-equity percentiles, maximum daily drawdown percentiles, and frequencies of loss, drawdown-threshold breaches and touching zero equity. It requires at least 30 observed days and supports up to 10,000; it never silently shortens the simulated horizon. Observed flat days remain in the sample and missing dates are not filled. Daily drawdowns omit intraday extremes; paths preserve historical cash sizing and continue arithmetically after zero without margin or liquidation rules. The extra-cost scenario is separate from this simulation. **Export robustness JSON** includes the assumptions and results; **Export PDF** includes the selected settings and robustness tables and charts.

These are diagnostics of existing history, not proof against overfitting or a probability of future profitability. The retrospective split is not an untouched holdout. Freeze rules before testing unseen data, track parameter searches, check nearby values and different regimes, validate fills and costs, and forward paper-test. The existing walk-forward mode refits parameters chronologically but cold-starts each slice, resets capital and may split a session; strategies requiring warmup or session continuity need those effects accounted for before interpreting its results.

**Walk-forward from a report.** The Robustness tab can rerun the report's original script and request against the current dataset. Enter a JSON parameter grid using names the Python script reads from `params`, for example `{"trend_ratio_threshold":"0.4,0.45,0.5"}` for the Raschke strategy. Keep quantity and risk settings fixed. Choose folds, training fraction and the training selection metric, then run validation. Each fold shows the training and subsequent test dates, selected parameters and test P&L; expand its training candidates to see nearby values and failed combinations. Candidates are selected on training data only. Test P&L is summed across independent folds with reset capital, not compounded into a portfolio curve. A saved dataset may have changed since the original run. **Save backtest** retains completed validation results and current robustness settings; validation JSON and report PDF exports also preserve them. Older saved reports without the original request still support the historical diagnostics but need a fresh normal backtest before walk-forward reruns.
...
- API run options accept `commission_per_unit` (account-currency cash per unit per side) and `commission_pct` (percentage of notional per side, so `0.01` means 0.01%). If both are supplied they are added. Keep broker fees out of strategy fill prices to avoid charging the same commission twice. Walk-forward and Monte Carlo retain the original run's fee options.
...
MY DATA is your library: files you import, files you download from LSE Data or Binance, and the ten samples. BACKTEST and the WORKSPACE tools read from here. RESEARCH > ANALYSIS can also read a library series that has open, high, low and close.
...
- **Download data.** The "Import market data" button on the library toolbar opens the provider selector. Choose **LSE Data** for the databank: the full recorded history behind the platform, unlocked by your key. Pick a dataset class (markets, options, series, reference), search the catalog (each instrument with its logo, name and years of coverage), choose a timeframe chip (tick and every candle interval), a range chip (Max, 1y, 180d, 90d, 30d, 7d, or custom dates bounded to the instrument's real coverage) and a library folder (default LSE), then Download to library. Candles and series import straight into the library; raw ticks, options and reference tables are saved as Parquet files on disk instead. A quota line shows what your key allows.
- **Binance.** Choose **Binance** in Import market data, then **Spot crypto**, **USD-M Futures** or **COIN-M Futures**. Search public instruments such as BTCUSDT or BTCUSD_PERP; no API key is needed. Selecting an instrument checks the first and latest completed candles for your chosen timeframe and shows the available dates, duration and approximate candle count. Counts are estimates from the history span, so exchange gaps can reduce the actual download. Coverage loads for the selected instrument rather than requesting every symbol at once.
- Choose 1m, 5m, 15m, 30m, 1h, 4h, 1d or 1w candles, a preset or custom UTC date range, and a folder (default Binance). **Max** downloads all history available from the public API; only completed candles are saved, and a date-only end includes that entire UTC day. Requests are paginated and rate-limited, and verified pages are checkpointed locally so repeating the same request resumes after interruption. All pages become one CSV named `BINANCE_<MARKET>_<SYMBOL>_<TIMEFRAME>`, keeping Spot, USD-M and COIN-M datasets separate. Futures history ends at delivery when a contract has expired. The instrument list follows Binance's current catalog; expired contracts removed from that catalog are not automatically discoverable. Binance network/location restrictions are reported in the dialog.
- Binance prices are in the quote asset. Spot and USD-M volume is in the base asset; COIN-M volume counts contracts. Futures imports retain contract type, settlement asset and contract size where supplied. These imports provide historical OHLCV: backtests do not simulate funding payments or margin liquidations, and the existing linear P&L engine does not reproduce inverse COIN-M settlement in cryptocurrency.
- **Samples.** GOLD, EURUSD, SPX500, BTCUSD (1h), SILVER, BRENT, USDJPY, NAS100, AAPL (1d) and USYIELDS (a daily Treasury curve series) are seeded on first launch into a Samples folder.
...
RESEARCH is the reading tab: a live wire of new quantitative finance papers with an in app reader, and an analysis desk that counts readings from bars already loaded. Articles need no key. The desk does not simulate a price.
...
- **ANALYSIS.** Eight readings, counted from the bars already loaded for the open chart, or from a library series that has open, high, low and close. Nothing on the page is simulated, and a short sample is labelled as a count rather than a law. **Room** asks whether a stop one median range away is reached before a target of one or two ranges, inside 20 bars; a bar that trades both is counted as the stop. **Late entry** measures how much of the next eight-bar extreme is already printed after waiting one, two or three bars past a close through the prior bar's high or low. **Break ledger** lists closes beyond the prior 20-bar range and whether any of the next three closes failed to stay beyond that level. **Breath** is how often a run of rising or falling closes continues, by length. **Wound** is how many bars until a close in the outer quarter of its range is traded back through that bar's open. **Quiet hours** is each UTC hour's share of the total high-low range. **Weather** marks each bar calm, push or shock from its range against the median, and what the next close did. **Round test** counts closes through a round number against closes that reject it. The method line under each reading states the rule. The chart toolbar's Analysis button opens the same page. A camera mark in the status strip, with no label, saves a picture of the reading or of the chart.
...
- Ctrl K (⌘K on a Mac) opens the command palette from any page. Type to filter, Enter to open, Escape to close. The same key closes it. Save a picture is in that list; the same action is the camera mark in the status strip, which has no label.
...
- **Analysis.** The RESEARCH desk that counts Room, Late entry, Break ledger, Breath, Wound, Quiet hours, Weather and Round test from bars already loaded. It does not simulate a price.
...
- **Databank.** The recorded history behind the platform, downloadable into your library by symbol and timeframe with your key (Import market data → LSE Data).
